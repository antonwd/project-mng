...
export default async function AuditPage({ searchParams }: { searchParams: Search }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <form className="grid gap-3 md:grid-cols-[1fr_1fr_auto] items-end" method="get">
        <div>
      ...
      <Card className="divide-y">
        {events.length === 0 && <div className="p-6 text-center text-muted-foreground text-sm">No events.</div>}
        {events.map((e) => (
          <div key={e.id} className="grid grid-cols-[140px_1fr] gap-3 p-3 text-sm">
